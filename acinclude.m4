...
[dnl 
dnl Get the cflags and libraries from the ncursesw6-config script, with
dnl fallback to ncursesw5-config for older environments.
dnl
...
AC_ARG_ENABLE(ncursestest, [  --disable-ncursestest       Do not try to compile and run a test ncurses program],
  ...
  if test x$ncurses_exec_prefix != x ; then
     ...
     if test x${NCURSES_CONFIG+set} != xset ; then
        if test -x "$ncurses_exec_prefix/bin/ncursesw6-config" ; then
  ...
  if test x$ncurses_prefix != x ; then
     ...
     if test x${NCURSES_CONFIG+set} != xset ; then
        if test -x "$ncurses_prefix/bin/ncursesw6-config" ; then
...
dnl checks the results of the discovered ncursesw-config script to some extent)
dnl
...
int main (int argc, char *argv[])
{
  ...
  if test "x$no_ncurses" = x ; then
     ...
     AC_MSG_RESULT(no)
     if test "$NCURSES_CONFIG" = "no" ; then
       echo "*** The ncursesw6-config or ncursesw5-config script installed by ncursesw could not be found"
       ...
       echo "*** full path to a working ncursesw-config script."
	 else
...
#include "ncurses.h"
        ...
        [ echo "*** The test program failed to compile or link. See the file config.log for the"
          ...
          echo "*** latter case, you may want to edit the detected ncursesw-config script:"
          echo "*** $NCURSES_CONFIG" ])
          CFLAGS="$ac_save_CFLAGS"
          LIBS="$ac_save_LIBS"
       fi
