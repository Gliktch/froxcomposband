...
static int _death_dump_screen_prompt(bool *quick, bool *silent)
{
    ...
    {
        screen_save();
        Term_clear_rect(rect(0, box_top, MIN(Term->wid, 82), MIN(20, Term->hgt - box_top)));

        if (strlen(p_ptr->died_from) > 34)
            strnfmt(killer, sizeof killer, "***** You have died. *****       Killer: %-.34s...", p_ptr->died_from);
        else
            strnfmt(killer, sizeof killer, "***** You have died. *****       Killer: %s", p_ptr->died_from);
        c_put_str(TERM_RED, death_title, row - 2, 2);
        c_put_str(TERM_WHITE, killer_label, row - 2, 2 + strlen(death_title));
        c_put_str(TERM_RED, killer + killer_start, row - 2, 2 + killer_start);
        ...
        put_str("Toggle silent deaths mode with [s]: ", row + 3, 2);
        c_put_str(TERM_ORANGE, "s", row + 3, 34);
        c_put_str(*silent ? TERM_L_GREEN : TERM_RED, *silent ? "ON" : "OFF", row + 3, 38);
